...
export const LiveCCTVProvider: React.FC<{
    ...
    async function loadData() {
      ...
      try {
        ...
        setInventoryProducts(
          invData.success ? invData.products || [] : []
        );
      } catch {
        setInventoryProducts([]);
      }
  ...
  const [customerWhatsAppSettings, setCustomerWhatsAppSettings] =
  ...
  const updateCustomerWhatsAppSettings = async (
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `HTTP ${response.status}: فشل حفظ إعدادات WhatsApp`
        );
      }
    } catch (e) {
      console.error(
  ...
  const dispatchWhatsAppAlert = async (
    ...
    if (!customerWhatsAppSettings.phoneNumber) {
      throw new Error(
        'أدخل رقم WhatsApp للعميل'
      );
    }

    if (!customerWhatsAppSettings.instanceName) {
      throw new Error(
    ...
    /*
     * Show toast only as informational UI.
     * The actual success message is shown AFTER
     * the backend confirms successful dispatch.
     */
    if (
      actionToTake === 'CALL'
    ) {
      ...
      setLastWhatsAppToast({
        message: `🚨 جارٍ إرسال إشعار وتفعيل نداء واتساب إلى (${customerWhatsAppSettings.phoneNumber})!`,
        type: 'CALL',
        visible: true,
      });
    } else {
      setLastWhatsAppToast({
        message: `💬 جارٍ إرسال رسالة إلى واتساب العميل (${customerWhatsAppSettings.phoneNumber})...`,
        type: 'MESSAGE',
        visible: true,
      });
    }
    ...
    /*
     * IMPORTANT:
     * Backend expects clientNumber, NOT phoneNumber.
     */
    const payload = {
      clientNumber:
        customerWhatsAppSettings.phoneNumber.trim(),

      instanceName:
        customerWhatsAppSettings.instanceName.trim(),
    ...
    console.log(
      ...
      {
        ...payload,
        // Do not expose full phone number in production logs
        clientNumber:
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          data?.error ||
          data?.message ||
          `فشل إرسال WhatsApp - HTTP ${res.status}`
        );
      }
      ...
      if (actionToTake === 'CALL') {
        setLastWhatsAppToast({
          message: `📞 تم تنفيذ طلب الاتصال بنجاح إلى (${customerWhatsAppSettings.phoneNumber})`,
          type: 'CALL',
          visible: true,
        });
      } else if (
        ...
        setLastWhatsAppToast({
          message: `🚨 تم إرسال الرسالة وتنفيذ طلب الاتصال بنجاح إلى (${customerWhatsAppSettings.phoneNumber})`,
          type: 'CALL',
          visible: true,
        });
      } else {
        setLastWhatsAppToast({
          message: `💬 تم إرسال رسالة WhatsApp بنجاح إلى (${customerWhatsAppSettings.phoneNumber})`,
          type: 'MESSAGE',
          visible: true,
        });
      }
