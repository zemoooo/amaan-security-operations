...
async function getEvolutionConnectionState(instanceName: string) {
  ...
  return { state: data?.instance?.state || data?.state || 'unknown', raw: data };
}

async function sendEvolutionText(instanceName: string, targetPhone: string, text: string) {
  const phone = cleanPhone(targetPhone);
  if (!phone) throw new Error('رقم WhatsApp غير صالح');
  if (!instanceName) throw new Error('اسم Instance مطلوب');
  const state = await getEvolutionConnectionState(instanceName);
  if (!['open', 'connected', 'CONNECTED'].includes(String(state.state))) {
    throw new Error(`جلسة WhatsApp غير متصلة. الحالة الحالية: ${state.state}`);
  }
  return evolutionRequest(`/message/sendText/${encodeURIComponent(instanceName)}`, {
    method: 'POST',
    body: JSON.stringify({ number: phone, textMessage: { text }, options: { delay: 500, presence: 'composing' } })
  });
}
...
app.get('/api/env/status', (req, res) => {
  ...
  const smtpConfigured = Boolean(process.env.SMTP_HOST && true);
  ...
  res.json({
    ...
    database: {
      ...
      connectionStringMasked: maskUrl(process.env.DATABASE_URL || ''),
    },
    redis: {
...
`;
      ...
      return res.status(503).json({ success: false, error: 'خدمة AI غير مهيأة. أضف GEMINI_API_KEY أو استخدم Edge Agent مع مزود AI حقيقي.' });
    }
  } catch (error: any) {
...
const DEFAULT_WHATSAPP_CONFIG = {
  phoneNumber: '', customerName: '', instanceName: '', enabled: true,
  alertMode: 'MESSAGE_ONLY', minSeverity: 'MEDIUM', callRingtoneEnabled: false,
  autoPlayVoiceBriefing: false, language: 'ar',
};
async function getWhatsAppConfig(req: any) {
  const tenantId = String(req.body?.tenantId || req.query?.tenantId || '').trim();
  if (!tenantId) return { ...DEFAULT_WHATSAPP_CONFIG };
  if (!supabaseAdmin) return { ...DEFAULT_WHATSAPP_CONFIG };
  const { data, error } = await supabaseAdmin.from('customer_whatsapp_settings').select('*').eq('tenant_id', tenantId).maybeSingle();
  if (error) throw error;
  return data ? {
    phoneNumber: data.phone_number || '', customerName: data.customer_name || '',
    instanceName: data.instance_name || '', enabled: data.enabled !== false,
    alertMode: 'MESSAGE_ONLY', minSeverity: data.min_severity || 'MEDIUM',
    callRingtoneEnabled: Boolean(data.call_ringtone_enabled), autoPlayVoiceBriefing: Boolean(data.auto_play_voice_briefing),
    language: data.language || 'ar',
    connectionState: data.connection_state || 'disconnected', connectedAt: data.connected_at || null, lastQrAt: data.last_qr_at || null,
  } : { ...DEFAULT_WHATSAPP_CONFIG };
}
async function saveWhatsAppConfig(req: any, partial: any) {
  const tenantId = String(req.body?.tenantId || req.query?.tenantId || '').trim();
  if (!tenantId) throw new Error('tenantId مطلوب لربط واتساب بالعميل.');
  const current = await getWhatsAppConfig(req);
  const merged = { ...current, ...partial };
  const { error } = await requireDatabase().from('customer_whatsapp_settings').upsert({
    tenant_id: tenantId, customer_email: String(req.body?.customerEmail || req.query?.customerEmail || '').trim().toLowerCase(),
    phone_number: String(merged.phoneNumber || '').trim(), customer_name: String(merged.customerName || '').trim(),
    instance_name: String(merged.instanceName || '').trim(), enabled: merged.enabled !== false,
    alert_mode: 'MESSAGE_ONLY', min_severity: merged.minSeverity || 'MEDIUM',
    language: merged.language || 'ar', call_ringtone_enabled: Boolean(merged.callRingtoneEnabled),
    auto_play_voice_briefing: Boolean(merged.autoPlayVoiceBriefing), updated_at: new Date().toISOString()
...
async function whatsappInstanceName(req: any) {
  const configured = await getWhatsAppConfig(req);
  const requested = String(req.body?.instanceName || req.query?.instanceName || '').trim();
  return requested || configured.instanceName || '';
}

app.get('/api/customer/whatsapp-settings', async (req, res) => {
  try {
    res.json({ success: true, settings: await getWhatsAppConfig(req), provider: 'EVOLUTION_API', configured: Boolean(evolutionBaseUrl() && process.env.EVOLUTION_API_KEY) });
  } catch (e: any) { res.status(500).json({ success: false, error: e.message }); }
});

app.post('/api/customer/whatsapp-settings', async (req, res) => {
  try {
    ...
    const settings = await saveWhatsAppConfig(req, { phoneNumber, customerName, instanceName, enabled, alertMode, minSeverity, callRingtoneEnabled, autoPlayVoiceBriefing, language });
    res.json({ success: true, settings, provider: 'EVOLUTION_API' });
  } catch (e: any) { res.status(500).json({ success: false, error: e.message }); }
});

app.post('/api/whatsapp/instance/create', async (req, res) => {
  try {
    let instanceName = await whatsappInstanceName(req);
    if (!instanceName) {
      const tenantId = String(req.body?.tenantId || '').trim();
      if (!tenantId) return res.status(400).json({ success: false, error: 'tenantId أو instanceName مطلوب' });
      instanceName = `aman_${tenantId.replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 36)}`;
    }
    ...
    let data: any;
    try {
      data = await evolutionRequest('/instance/create', { method: 'POST', body: JSON.stringify({ instanceName, token: crypto.randomBytes(16).toString('hex'), integration: 'WHATSAPP-BAILEYS', qrcode: true, groupsIgnore: true, ...(webhookUrl ? { webhook: webhookUrl, webhookByEvents: false, webhookBase64: false, events: ['QRCODE_UPDATED','CONNECTION_UPDATE','SEND_MESSAGE','MESSAGES_UPSERT','MESSAGES_UPDATE'], ...(webhookHeaders ? { headers: webhookHeaders } : {}) } : {}) }) });
    } catch (e: any) {
      const msg = String(e?.message || '');
      if (!/already|exists|existente|409|conflict/i.test(msg)) throw e;
      data = await evolutionRequest(`/instance/connect/${encodeURIComponent(instanceName)}`, { method: 'GET' });
    }
    await saveWhatsAppConfig(req, { instanceName, enabled: true });
    const qr = extractEvolutionQr(data);
    let state = data?.instance?.state || data?.state || 'unknown';
    try { state = (await getEvolutionConnectionState(instanceName)).state; } catch {}
    await requireDatabase().from('customer_whatsapp_settings').update({ connection_state: state, last_qr_at: qr.base64 ? new Date().toISOString() : null, updated_at: new Date().toISOString() }).eq('tenant_id', String(req.body?.tenantId || '').trim());
    res.json({ success: true, instanceName, state, ...qr, evolution: data });
...
app.get('/api/whatsapp/qr/:instanceName', async (req, res) => {
  try {
    ...
    let state = data?.instance?.state || data?.state || 'unknown';
    try { state = (await getEvolutionConnectionState(instanceName)).state; } catch {}
    res.json({ success: true, instanceName, state, ...qr });
  } catch (error: any) { res.status(502).json({ success: false, error: error.message }); }
});
...
app.get('/api/whatsapp/status/:instanceName', async (req, res) => {
  try {
    const instanceName = String(req.params.instanceName);
    const stateData = await getEvolutionConnectionState(instanceName);
    const tenantId = String(req.query?.tenantId || '').trim();
    if (supabaseAdmin && tenantId) {
      await supabaseAdmin.from('customer_whatsapp_settings').update({ connection_state: stateData.state, connected_at: stateData.state === 'open' ? new Date().toISOString() : null, updated_at: new Date().toISOString() }).eq('tenant_id', tenantId).eq('instance_name', instanceName);
    }
    res.json({ success: true, instanceName, state: stateData.state, evolution: stateData.raw });
  } catch (error: any) { res.status(502).json({ success: false, error: error.message }); }
});

app.post('/api/whatsapp/instance/logout', async (req, res) => {
  try {
    const instanceName = await whatsappInstanceName(req);
...
app.get('/api/notifications/whatsapp/logs', async (req, res) => {
  try {
    const tenantId = String(req.query?.tenantId || '').trim();
    if (supabaseAdmin && tenantId) {
      const { data, error } = await supabaseAdmin.from('whatsapp_events').select('*').eq('tenant_id', tenantId).order('created_at', { ascending: false }).limit(100);
      if (!error) {
        const logs = (data || []).filter((x: any) => x.event_type === 'MESSAGE_SENT').map((x: any) => x.payload);
        return res.json({ success: true, logs });
      }
    }
    return res.json({ success: true, logs: whatsappDispatchHistory.slice(0, 100) });
...
app.post('/api/whatsapp/webhook', async (req, res) => {
  try {
    ...
    if (supabaseAdmin) {
      const tenantId = req.body?.data?.tenantId || req.body?.tenantId || null;
      await supabaseAdmin.from('whatsapp_events').insert({ id: crypto.randomUUID(), tenant_id: tenantId, event_type: event, payload: req.body, created_at: new Date().toISOString() });
      const instanceName = req.body?.instance || req.body?.data?.instance || req.body?.instanceName || null;
      const state = req.body?.data?.state || req.body?.state || req.body?.data?.status || null;
      if (tenantId && instanceName && state) await supabaseAdmin.from('customer_whatsapp_settings').update({ connection_state: String(state).toLowerCase(), connected_at: String(state).toLowerCase() === 'open' ? new Date().toISOString() : null, updated_at: new Date().toISOString() }).eq('tenant_id', tenantId).eq('instance_name', instanceName);
    }
...
const LICENSE_SALT = String(process.env.LICENSE_SECRET_SALT || '').trim();
if (!LICENSE_SALT) console.warn('[AMAN] LICENSE_SECRET_SALT is not configured; activation keys cannot be generated securely.');

function generateActivationKey(machineId: string, period: string): string {
  const cleanMachine = String(machineId || '').trim().toUpperCase();
  if (!LICENSE_SALT) throw new Error('LICENSE_SECRET_SALT غير مضبوط على الخادم.');
...
// Persistent subscriber/license datastore (Supabase is the source of truth).
// No demo customers, fake phone numbers, passwords, or generated tenants are created here.
function requireDatabase() {
  if (!supabaseAdmin) throw new Error('Supabase غير مضبوط. أضف SUPABASE_URL و SUPABASE_SERVICE_ROLE_KEY.');
...
function publicSubscriber(row: any) {
  ...
  return {
    ...
    phone: row.phone || '', companyName: row.company_name || '',
    machineId: row.machine_id || '', role: row.role, emailVerified: Boolean(row.email_verified),
    totalPaid: Number(row.total_paid || 0), registeredAt: row.registered_at,
    lastActiveAt: row.last_active_at, status: row.status,
    currentLicense: row.current_license || null,
  };
}
function publicLicense(row: any) {
  if (!row) return null;
  return {
    id: row.id, machineId: row.machine_id, activationKey: row.activation_key,
    period: row.period, periodLabelAr: row.period_label_ar,
    customerEmail: row.customer_email || '', customerName: row.customer_name || '',
    customerPhone: row.customer_phone || '', companyName: row.company_name || '',
    amountPaid: Number(row.amount_paid || 0), currency: row.currency || 'SAR',
    status: row.status, activatedAt: row.activated_at, expiresAt: row.expires_at,
    notes: row.notes || '', isLocked: Boolean(row.is_locked),
  };
}
async function getSubscriberByEmail(email: string) {
  const db = requireDatabase();
  const { data, error } = await db.from('subscribers').select('*').eq('email', email.toLowerCase()).maybeSingle();
  if (error) throw error;
  return data;
}
async function getSubscriberById(id: string) {
  const db = requireDatabase();
  const { data, error } = await db.from('subscribers').select('*').eq('id', id).maybeSingle();
  if (error) throw error;
  return data;
}
async function getSubscriberWithLicense(id: string) {
  const db = requireDatabase();
  const subscriber = await getSubscriberById(id);
  if (!subscriber) return null;
  const { data: license, error } = await db.from('subscriber_licenses').select('*')
    .eq('subscriber_id', id).order('created_at', { ascending: false }).limit(1).maybeSingle();
  if (error) throw error;
  return { ...subscriber, current_license: publicLicense(license) };
}
async function saveSubscriberLicense(subscriberId: string, license: any) {
  const db = requireDatabase();
  const { data, error } = await db.from('subscriber_licenses').insert({
    id: license.id, subscriber_id: subscriberId, machine_id: license.machineId,
    activation_key: license.activationKey, period: license.period,
    period_label_ar: license.periodLabelAr, customer_email: license.customerEmail,
    customer_name: license.customerName, customer_phone: license.customerPhone,
    company_name: license.companyName, amount_paid: license.amountPaid,
    currency: license.currency, status: license.status, activated_at: license.activatedAt,
    expires_at: license.expiresAt, notes: license.notes, is_locked: license.isLocked
  }).select('*').single();
  if (error) throw error;
...
async function ensureConfiguredSuperAdmin() {
  ...
  const name = String(process.env.SUPER_ADMIN_NAME || 'مدير النظام').trim();
  if (!email || !password) return;
  const db = requireDatabase();
  const existing = await getSubscriberByEmail(email);
  if (existing) {
    if (existing.role !== 'SUPER_ADMIN') await db.from('subscribers').update({ role: 'SUPER_ADMIN', email_verified: true }).eq('id', existing.id);
    return;
  }
  const tenantId = `tenant-system-${crypto.randomUUID()}`;
  const subscriberId = `sub-${crypto.randomUUID()}`;
  await db.from('tenants').insert({
    id: tenantId, name: name, name_en: name, status: 'ACTIVE',
    camera_limit: 0, device_limit: 0, employee_limit: 0, retention_days: 365,
    modules: { all: true }
  });
  await db.from('subscribers').insert({
    id: subscriberId, tenant_id: tenantId, email, password_hash: hashPassword(password),
    name, phone: '', company_name: '', machine_id: '', role: 'SUPER_ADMIN',
    email_verified: true, email_verified_at: new Date().toISOString(),
    total_paid: 0, registered_at: new Date().toISOString(), last_active_at: new Date().toISOString(), status: 'ACTIVE'
  });
  await db.from('users').insert({
    id: subscriberId, tenant_id: tenantId, name, email, role: 'SUPER_ADMIN',
    phone: '', permissions: ['all_superadmin_rights','manage_subscriptions','generate_license_keys'], is_active: true
  });
}
...
async function createEmailVerification(subscriberId: string, email: string, name: string) {
  ...
  const { error: tokenError } = await db.from('email_verification_tokens').insert({
    token, subscriber_id: subscriberId, expires_at: expiresAt
  });
  if (tokenError) throw tokenError;

  const appUrl = String(process.env.APP_URL || '').replace(/\/$/, '');
  if (!appUrl) throw new Error('APP_URL غير مضبوط.');
  const verifyUrl = `${appUrl}/api/auth/verify-email?token=${token}`;
  const transport = getMailTransport();
  if (!transport) throw new Error('خدمة البريد غير مضبوطة في Render.');
  ...
  await transport.sendMail({
    ...
    html: `<div dir="rtl" style="font-family:Arial;line-height:1.8"><h2>تأكيد البريد الإلكتروني</h2><p>مرحباً ${name}،</p><p><a href="${verifyUrl}" style="display:inline-block;padding:12px 20px;background:#0891b2;color:#fff;text-decoration:none;border-radius:8px">تأكيد البريد الإلكتروني</a></p><p>الرابط صالح لمدة 24 ساعة.</p></div>`
  });
}

// 8.1 Real customer registration/login/email verification — persistent in Supabase
app.post('/api/auth/register', async (req, res) => {
  try {
    const db = requireDatabase();
    const { email, password, name, phone, companyName, machineId } = req.body || {};
    if (!email || !password || !name) return res.status(400).json({ error: 'البريد الإلكتروني وكلمة المرور والاسم مطلوبة.' });
    ...
    if (existing) return res.status(409).json({ error: 'هذا البريد الإلكتروني مسجل مسبقاً. استخدم تسجيل الدخول.' });

    const tenantId = `tenant-${crypto.randomUUID()}`;
    const subscriberId = `sub-${crypto.randomUUID()}`;
    const cleanMachine = String(machineId || '').trim().toUpperCase() || `AMAN-${crypto.randomUUID().slice(0,8).toUpperCase()}`;
    const now = new Date().toISOString();

    const { error: tenantError } = await db.from('tenants').insert({
      id: tenantId, name: String(companyName || name).trim(), name_en: String(companyName || name).trim(),
      status: 'TRIAL', camera_limit: 4, device_limit: 1, employee_limit: 50, retention_days: 7,
      modules: { inventoryVision: true, attendanceTracking: true, theftDetection: true, behaviorAnalytics: true, whatsappAlerts: true, windowsAgent: true }
    });
    if (tenantError) throw tenantError;

    const { error: subError } = await db.from('subscribers').insert({
      id: subscriberId, tenant_id: tenantId, email: cleanEmail, password_hash: hashPassword(password),
      name: String(name).trim(), phone: String(phone || '').trim(), company_name: String(companyName || '').trim(),
      machine_id: cleanMachine, role: 'OWNER', email_verified: false, total_paid: 0,
      registered_at: now, last_active_at: now, status: 'TRIAL'
    });
    if (subError) {
    ...
    const { error: userError } = await db.from('users').insert({
      id: subscriberId, tenant_id: tenantId, name: String(name).trim(), email: cleanEmail,
      role: 'OWNER', phone: String(phone || '').trim(), permissions: ['manage_cameras','view_all_reports'],
      is_active: true
    });
    if (userError) throw userError;
    await createEmailVerification(subscriberId, cleanEmail, String(name).trim());

    const trial = {
      id: `lic-${crypto.randomUUID()}`, machineId: cleanMachine, activationKey: generateActivationKey(cleanMachine, '2_DAYS'),
      period: '2_DAYS', periodLabelAr: 'يومان (فترة تجريبية)', customerEmail: cleanEmail,
      customerName: String(name).trim(), customerPhone: String(phone || '').trim(),
      companyName: String(companyName || '').trim(), amountPaid: 0, currency: 'SAR', status: 'ACTIVE',
      activatedAt: now, expiresAt: calculateExpiryIso('2_DAYS'), notes: 'فترة تجريبية عند التسجيل', isLocked: false
    };
    await saveSubscriberLicense(subscriberId, trial);

    res.json({
      success: true, requiresEmailVerification: true,
      message: 'تم إنشاء الحساب. تحقق من بريدك الإلكتروني ثم سجّل الدخول.',
      user: { id: subscriberId, tenantId, email: cleanEmail, name: String(name).trim(), role: 'OWNER', emailVerified: false }
    });
  } catch (error: any) {
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    const cleanEmail = String(email || '').trim().toLowerCase();
    if (!cleanEmail || !password) return res.status(400).json({ error: 'البريد الإلكتروني وكلمة المرور مطلوبان.' });
    const subscriber = await getSubscriberByEmail(cleanEmail);
    if (!subscriber || !verifyPassword(password, subscriber.password_hash)) {
      return res.status(401).json({ error: 'البريد الإلكتروني أو كلمة المرور غير صحيحة.' });
    }
    if (!subscriber.email_verified) {
      return res.status(403).json({ requiresEmailVerification: true, error: 'يجب تأكيد البريد الإلكتروني أولاً.' });
    }
    const full = await getSubscriberWithLicense(subscriber.id);
    await requireDatabase().from('subscribers').update({ last_active_at: new Date().toISOString() }).eq('id', subscriber.id);
    res.json({
      success: true, message: `أهلاً بك ${subscriber.name}`,
      user: publicSubscriber(full), license: full?.current_license || null,
      token: `aman_${crypto.randomUUID()}`
    });
  } catch (error: any) {
...
app.get('/api/auth/verify-email', async (req, res) => {
  try {
    ...
    const db = requireDatabase();
    const { data: record, error } = await db.from('email_verification_tokens').select('*').eq('token', token).maybeSingle();
    if (error) throw error;
    if (!record || new Date(record.expires_at).getTime() < Date.now()) {
      return res.status(400).send('<html dir="rtl"><body style="font-family:Arial;text-align:center;padding:50px"><h2>رابط التحقق غير صالح أو منتهي</h2></body></html>');
    }
    await db.from('subscribers').update({ email_verified: true, email_verified_at: new Date().toISOString() }).eq('id', record.subscriber_id);
    await db.from('email_verification_tokens').delete().eq('token', token);
    const appUrl = String(process.env.APP_URL || '').replace(/\/$/, '');
    res.redirect(`${appUrl}/?email_verified=1`);
  } catch (error: any) {
    res.status(500).send(`<html dir="rtl"><body style="font-family:Arial;text-align:center;padding:50px"><h2>تعذر التحقق من البريد</h2><p>${String(error.message || '')}</p></body></html>`);
  }
});
...
app.post('/api/auth/resend-verification', async (req, res) => {
  try {
    ...
    if (!subscriber) return res.status(404).json({ error: 'الحساب غير موجود.' });
    if (subscriber.email_verified) return res.json({ success: true, alreadyVerified: true, message: 'البريد مؤكد بالفعل.' });
    ...
    res.status(500).json({ error: error.message || 'تعذر إرسال رابط التحقق.' });
  }
});
...
app.get('/api/license/status', async (req, res) => {
  try {
    const machineId = String(req.query.machineId || '').trim().toUpperCase();
    if (!machineId) return res.status(400).json({ error: 'machineId مطلوب.' });
    const db = requireDatabase();
    const { data: license, error } = await db.from('subscriber_licenses').select('*').eq('machine_id', machineId)
      .order('created_at', { ascending: false }).limit(1).maybeSingle();
    if (error) throw error;
    if (!license) return res.json({ success: true, machineId, isLocked: true, status: 'NOT_REGISTERED', license: null, daysRemaining: 0, hoursRemaining: 0 });
    const diffMs = new Date(license.expires_at).getTime() - Date.now();
    const expired = diffMs <= 0 || license.status === 'EXPIRED' || license.status === 'REVOKED';
    if (expired && !license.is_locked) {
      await db.from('subscriber_licenses').update({ is_locked: true, status: 'EXPIRED' }).eq('id', license.id);
      await db.from('subscribers').update({ status: 'EXPIRED' }).eq('id', license.subscriber_id);
    }
    return res.json({
      success: true, machineId, isLocked: expired || Boolean(license.is_locked),
      status: expired ? 'EXPIRED' : license.status, license: publicLicense(license),
      daysRemaining: Math.max(0, Math.floor(diffMs / 86400000)),
      hoursRemaining: Math.max(0, Math.floor(diffMs / 3600000)),
      expiresAt: license.expires_at,
      supportEmail: process.env.SUPPORT_EMAIL || ''
    });
  } catch (error: any) { res.status(500).json({ error: error.message || 'تعذر فحص الترخيص.' }); }
});

app.post('/api/license/activate', async (req, res) => {
  try {
    const { machineId, activationKey } = req.body || {};
    if (!machineId || !activationKey) return res.status(400).json({ error: 'machineId و activationKey مطلوبان.' });
    const cleanMachine = String(machineId).trim().toUpperCase();
    const cleanKey = String(activationKey).trim().toUpperCase();
    const verified = verifyActivationKey(cleanMachine, cleanKey);
    if (!verified.valid || !verified.period) return res.status(400).json({ success: false, error: verified.reason });
    const db = requireDatabase();
    const { data: subscriber } = await db.from('subscribers').select('*').eq('machine_id', cleanMachine).maybeSingle();
    if (!subscriber) return res.status(404).json({ success: false, error: 'لا يوجد مشترك مسجل بهذا الجهاز.' });
    const now = new Date().toISOString();
    const license = await saveSubscriberLicense(subscriber.id, {
      id: `lic-${crypto.randomUUID()}`, machineId: cleanMachine, activationKey: cleanKey,
      period: verified.period, periodLabelAr: getPeriodLabelAr(verified.period),
      customerEmail: subscriber.email, customerName: subscriber.name, customerPhone: subscriber.phone || '',
      companyName: subscriber.company_name || '', amountPaid: Number(subscriber.total_paid || 0),
      currency: 'SAR', status: 'ACTIVE', activatedAt: now, expiresAt: calculateExpiryIso(verified.period),
      notes: `تفعيل بواسطة مفتاح صادر من الإدارة`, isLocked: false
    });
    await db.from('subscribers').update({ status: verified.period === '2_DAYS' ? 'TRIAL' : 'ACTIVE' }).eq('id', subscriber.id);
    res.json({ success: true, unlocked: true, license, expiresAt: license.expiresAt });
  } catch (error: any) { res.status(500).json({ error: error.message || 'فشل التفعيل.' }); }
});

app.get('/api/admin/subscribers', async (req, res) => {
  try {
    const db = requireDatabase();
    const { data: subs, error } = await db.from('subscribers').select('*').order('registered_at', { ascending: false });
    if (error) throw error;
    const { data: licenses } = await db.from('subscriber_licenses').select('*').order('created_at', { ascending: false });
    const subscribers = (subs || []).map((s: any) => {
      const lic = (licenses || []).find((l: any) => l.subscriber_id === s.id);
      return { ...publicSubscriber({ ...s, current_license: publicLicense(lic) }) };
    });
    const totalRevenue = subscribers.reduce((n: number, s: any) => n + Number(s.totalPaid || 0), 0);
    res.json({
      success: true, subscribers,
      financialStats: {
        totalRevenue, currency: 'SAR',
        activeSubscribersCount: subscribers.filter((s: any) => s.status === 'ACTIVE').length,
        expiredCount: subscribers.filter((s: any) => s.status === 'EXPIRED').length,
        trialCount: subscribers.filter((s: any) => s.status === 'TRIAL').length,
        totalKeysGenerated: (licenses || []).length
      },
      licensesHistory: (licenses || []).map(publicLicense)
    });
  } catch (error: any) { res.status(500).json({ error: error.message || 'تعذر تحميل المشتركين.' }); }
});

app.post('/api/admin/generate-key', async (req, res) => {
  try {
    const { machineId, period = '1_MONTH', customerEmail = '', customerName = '', customerPhone = '', companyName = '', amountPaid = 0, currency = 'SAR', notes = '' } = req.body || {};
    if (!machineId) return res.status(400).json({ error: 'machineId مطلوب.' });
    const cleanMachine = String(machineId).trim().toUpperCase();
    const key = generateActivationKey(cleanMachine, period);
    const license = {
      id: `lic-${crypto.randomUUID()}`, machineId: cleanMachine, activationKey: key, period,
      periodLabelAr: getPeriodLabelAr(period), customerEmail: String(customerEmail).trim().toLowerCase(),
      customerName: String(customerName).trim(), customerPhone: String(customerPhone).trim(),
      companyName: String(companyName).trim(), amountPaid: Number(amountPaid) || 0, currency: String(currency || 'SAR'),
      status: 'ACTIVE', activatedAt: new Date().toISOString(), expiresAt: calculateExpiryIso(period),
      notes: String(notes || ''), isLocked: false
    };
    const db = requireDatabase();
    let subscriber = null;
    if (license.customerEmail) subscriber = await getSubscriberByEmail(license.customerEmail);
    if (!subscriber) {
      return res.status(404).json({ error: 'لا يوجد حساب مشترك بهذا البريد. يجب تسجيل العميل أولاً.' });
    }
    await db.from('subscribers').update({
      machine_id: cleanMachine, name: license.customerName || subscriber.name,
      phone: license.customerPhone || subscriber.phone, company_name: license.companyName || subscriber.company_name,
      total_paid: Number(subscriber.total_paid || 0) + license.amountPaid,
      status: period === '2_DAYS' ? 'TRIAL' : 'ACTIVE'
    }).eq('id', subscriber.id);
    const saved = await saveSubscriberLicense(subscriber.id, license);
    res.json({ success: true, license: saved, activationKey: key });
  } catch (error: any) { res.status(500).json({ error: error.message || 'فشل توليد المفتاح.' }); }
});

app.post('/api/license/master-activate', async (req, res) => {
  return res.status(403).json({ success: false, error: 'تفعيل المدير العام يتم من خلال حساب SUPER_ADMIN موجود في Supabase؛ لا توجد حسابات أو أرقام افتراضية.' });
});
...
app.post('/api/license/start-trial', async (req, res) => {
  try {
    ...
    const email = String(req.body?.email || '').trim().toLowerCase();
    if (!machineId || !email) return res.status(400).json({ error: 'البريد الإلكتروني وmachineId مطلوبان.' });
    const subscriber = await getSubscriberByEmail(email);
    if (!subscriber) return res.status(404).json({ error: 'سجل الحساب أولاً.' });
    const db = requireDatabase();
    const license = {
      id: `lic-${crypto.randomUUID()}`, machineId, activationKey: generateActivationKey(machineId, '2_DAYS'),
      period: '2_DAYS', periodLabelAr: 'يومان (فترة تجريبية)', customerEmail: subscriber.email,
      customerName: subscriber.name, customerPhone: subscriber.phone || '', companyName: subscriber.company_name || '',
      amountPaid: 0, currency: 'SAR', status: 'ACTIVE', activatedAt: new Date().toISOString(),
      expiresAt: calculateExpiryIso('2_DAYS'), notes: 'فترة تجريبية مرتبطة بالحساب', isLocked: false
    };
    const saved = await saveSubscriberLicense(subscriber.id, license);
    await db.from('subscribers').update({ machine_id: machineId, status: 'TRIAL' }).eq('id', subscriber.id);
    res.json({ success: true, unlocked: true, license: saved });
  } catch (error: any) { res.status(500).json({ error: error.message || 'فشل بدء التجربة.' }); }
});

// Inventory master data (persistent, no demo records)
app.get('/api/inventory/products', async (req, res) => {
  try {
    const tenantId = String(req.query.tenantId || '').trim();
    if (!tenantId) return res.status(400).json({ success: false, error: 'tenantId مطلوب' });
    const { data, error } = await requireDatabase().from('inventory_products').select('*').eq('tenant_id', tenantId).order('created_at', { ascending: false });
    if (error) throw error;
...
app.post('/api/inventory/products', async (req, res) => {
  try {
    const b = req.body || {};
    if (!b.tenantId || !b.name) return res.status(400).json({ success: false, error: 'tenantId واسم الصنف مطلوبان' });
    const product = {
      id: b.id || `prod-${crypto.randomUUID()}`, tenant_id: b.tenantId, name: String(b.name).trim(),
      sku: String(b.sku || '').trim(), barcode: String(b.barcode || '').trim(),
      warehouse: String(b.warehouse || '').trim(), zone: String(b.zone || '').trim(),
      camera_name: String(b.cameraName || '').trim(), unit: String(b.unit || 'قطعة').trim(),
      units_per_carton: Math.max(1, Number(b.unitsPerCarton || 1)), expected_quantity: Math.max(0, Number(b.expectedQuantity || 0)),
      ai_detected_quantity: Math.max(0, Number(b.aiDetectedQuantity || 0)), confidence: 0, difference: 0,
      outgoing_count_today: 0, incoming_count_today: 0, last_count_timestamp: new Date().toISOString(),
      low_stock_threshold: Math.max(0, Number(b.lowStockThreshold || 0)), status: 'NORMAL'
    };
    const { data, error } = await requireDatabase().from('inventory_products').insert(product).select('*').single();
    if (error) throw error;
...
app.post('/api/agent/security-event', requireAgent, async (req, res) => {
  try {
    ...
    if (eligible && shouldNotify(key, severity, Number(process.env.AI_ALERT_COOLDOWN_SECONDS || 600))) {
      const agentWhatsAppConfig = await getWhatsAppConfig({ body: { tenantId, customerEmail: b.customerEmail } });
